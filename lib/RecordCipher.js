...
export class RecordCipher {
  constructor({
    ...
    this.currentKekId = null;
    this.keks = keks;
    this.kekLoader = kekLoader;
    this.encoding = encoding;
    // only used during KEK lazy-load
    this._loadingKeksCache = new LruCache({
      max: 1000,
      ttl: 5000,
      // throw out the cache entry after the KEK loads
  ...
  async getKek({id} = {}) {
    // get pre-loaded key
    const secretKey = await this.keks.get(id);
    if(secretKey) {
      return secretKey;
    }

    // lazy-load key if possible
    if(this.kekLoader) {
      const secretKey = await this._loadingKeksCache.memoize({
        key: id,
        fn: () => _loadKekWithLoader({id, loader: this.kekLoader})
      })
      if(secretKey) {
        this.keks.set(id, secretKey);
        return secretKey;
  ...
  async setCurrentKek({id} = {}) {
    if(id !== null) {
      // ensure KEK exists
      await this.getKek({id});
    }
   ...
   * @param {Function} [options.kekLoader] - A function that can be used to
   *   lazy load KEKs.
   ...
   */
  static async create({
    currentKekId, keks = [], kekLoader, encoding = 'cbor'
  } = {}) {
    ...
    if(!(kekLoader === undefined || typeof kekLoader === 'function')) {
      throw new TypeError('"kekLoader" must be a function.');
    }
    if(!(encoding === 'cbor' || encoding === 'json')) {
      throw new Error('"encoding" must be "cbor" or "json".');
    }

    const recordCipher = new RecordCipher({kekLoader, encoding});
    ...
    if(currentKekId !== null) {
      await recordCipher.setCurrentKek({id: currentKekId ?? firstId ?? null});
    }
...
async function _loadKekWithLoader({id, loader}) {
  const result = await loader({id});
  if(!(result?.secretKey || result?.secretKeyMultibase)) {
    return null;
  }
  const secretKey = result.secretKey ?? _loadKek(result.secretKeyMultibase);
  return secretKey;
}
